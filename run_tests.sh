...
java -ea groupfs.tests.FilesystemTests 2>&1 | tee tests.log
...
readonly MOUNT="`readlink -f groupfs_mount.sh` $ORIGIN $MP"
...
cp \$ORIGIN/TMP_FILE "\$MP/log/messages #log"
diff \$ORIGIN/TMP_FILE "\$MP/log/messages #log"

mkdir \$MP/block
A=\$SCRATCH/random-junk
B="\$MP/block/random #block"
B_swp="\$MP/block/random.swp #block"
...
dd if="\$B" of=\$C count=1 skip=10 bs=1 >/dev/null 2>&1
diff \$A.swp \$C
...
dd if="\$B" of="\$B_swp" skip=100 count=100 bs=1 >/dev/null 2>&1
diff \$A.swp "\$B_swp"
dd if=\$A.swp of=\$A seek=20 count=100 skip=3 bs=1 conv=notrunc >/dev/null 2>&1
dd if="\$B_swp" of="\$B" seek=20 count=100 skip=3 bs=1 conv=notrunc >/dev/null 2>&1
diff \$A "\$B"
